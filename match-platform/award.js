...
(function (global) {
  ...
  function buildAwardSVG(top3, opts) {
    ...
    const W = 1080, H = 1350;
    const deck = [
      { x: 340, y: 640, w: 400, h: 470, c: '#C9A24B' },
      { x: 90, y: 800, w: 340, h: 310, c: '#9AA7B4' },
      { x: 650, y: 800, w: 340, h: 310, c: '#B5793B' }
    ];
    ...
    top3.slice(0, 3).forEach((p, i) => {
      ...
      <circle cx="${d.x + d.w / 2}" cy="${d.y - 70}" r="74" fill="#fff" stroke="#1f2d22" stroke-width="4"/>
      <text x="${d.x + d.w / 2}" y="${d.y - 48}" font-size="72" font-weight="800" text-anchor="middle" fill="#1f2d22">${i + 1}</text>
      <text x="${d.x + d.w / 2}" y="${d.y + 70}" font-size="50" font-weight="800" text-anchor="middle" fill="#fff">${label[i]}</text>
      <text x="${d.x + d.w / 2}" y="${d.y + 140}" font-size="46" font-weight="700" text-anchor="middle" fill="#fff">${escapeXml(name)}</text>
      <text x="${d.x + d.w / 2}" y="${d.y + 196}" font-size="30" text-anchor="middle" fill="#f4ead2">${escapeXml(sub)}</text>`;
    });
    ...
    return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${W} ${H}" width="${W}" height="${H}">
      ...
      <text x="${W / 2}" y="92" font-size="40" font-weight="700" text-anchor="middle" fill="${t.accent}" letter-spacing="2">⭐ 星羿羽毛球馆 · 未来赛事板块</text>
      <text x="${W / 2}" y="164" font-size="76" font-weight="900" text-anchor="middle" fill="#ffffff">${escapeXml(title)}</text>
      <text x="${W / 2}" y="222" font-size="34" text-anchor="middle" fill="${t.accent2}">${escapeXml(opts.tag || '以球会友 · 积分常驻')}</text>
      ${cards}
      <text x="${W / 2}" y="${H - 96}" font-size="34" text-anchor="middle" fill="#cfe9d8">${escapeXml(date)}</text>
      <text x="${W / 2}" y="${H - 52}" font-size="30" text-anchor="middle" fill="#8fb7a3">${escapeXml(venue)}</text>
    </svg>`;
  }
